...
from dinov3_extractor import DINOv3FeatureExtractor, DINOV3_MEAN, DINOV3_STD
...
def _filter_imagefolder_by_manipulation(dataset, allowed_manips, short_map):
    ...
    for path, target in dataset.samples:
        ...
        # only Qwen prompts have the trailing _0, _1, ...
    ...
    dataset.targets = [t for _, t in filtered]
...
def forward_features(
    ...
    if model_name == "dinov3_vitb16_featmap":
        return forward_dino_featmap(
            model,
            x,
            target_layers,
        )
...
def extract_and_store_features(
    ...
    feature_shapes = {f"feat{i}": fixed_feats[i].shape[1:] for i in target_layers}
    ...
    for batch_idx, (images, targets, paths) in enumerate(dataloader):
        ...
        for p in paths:
            ...
            if full_manip.startswith("qwen_"):
                full_manip = re.sub(r"_\d+$", "", full_manip)
    ...
    print("\nManipulations found:")
    print(sorted(manip_counter.keys()))
...
def _filter_imagefolder_by_subdir(dataset, subdir_name):
    ...
    if not subdir_name:
        return dataset
    filtered = [(p, y) for p, y in dataset.samples if subdir_name in p.split(os.sep)]
    dataset.samples = filtered
...
for model_name in cfg["models"]:
    for dataset in datasets:
        ...
        if model_name == "convnext_base.fb_in22k_ft_in1k":
            ...
            model = (
                timm.create_model(model_name, pretrained=True, features_only=False)
                .eval()
                .to(device)
            )
        ...
        elif model_name == "dinov3_vitb16_featmap":
            model = (
            ...
            feature_save_dir_train = "dinov3_features/augmented_train"
            feature_save_dir_test = "dinov3_features/augmented_test"
        ...
        for manipulation_model_dir in manipulation_model_dirs:
            ...
            if "train" in extract_from_splits:
                train_image_dir = os.path.join(root_dir, "images/augmented_train")
                ...
                train_dataset = _filter_imagefolder_by_manipulation(
                    train_dataset,
                    ALLOWED_MANIPULATIONS,
                    short_map,
                )

                train_feature_base = os.path.join(root_dir, feature_save_dir_train)
            ...
            if "test" in extract_from_splits:
                test_image_dir = os.path.join(root_dir, "images/augmented_test")
                ...
                test_dataset = _filter_imagefolder_by_manipulation(
                    test_dataset,
                    ALLOWED_MANIPULATIONS,
                    short_map,
                )

                test_feature_base = os.path.join(root_dir, feature_save_dir_test)
