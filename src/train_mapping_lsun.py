...
CONFIG_PATH = PROJECT_ROOT / "config" / "train_mapping_lsun.yaml"
...
def fit_closed_form_model_if_needed(model, data_module, device):
    """
    Fit ridge or Procrustes on the training split only.
    ...
    if not hasattr(data_module, "paired_train_dataset"):
    ...
    with torch.no_grad():
        for orig_features, target_features, _ in train_loader:
            orig_features = orig_features.to(
                device,
            ...
            B, C_in, H, W = orig_features.shape
            B_y, C_out, H_y, W_y = target_features.shape

            if B != B_y or H != H_y or W != W_y:
                raise ValueError(
                    "Input and target feature maps must have matching "
            ...
            if input_dim is None:
                ...
                if input_dim != model.input_dim:
                    raise ValueError(
                        f"Collected input dimension {input_dim} does not "
                        f"match configured input_dim={model.input_dim}."
                    )

                if output_dim != model.output_dim:
                    raise ValueError(
                        f"Collected output dimension {output_dim} does not "
                        f"match configured output_dim={model.output_dim}."
                    )
    ...
    if n_samples == 0:
        raise RuntimeError(
            "Training loader produced no feature vectors."
        )

    x_mean = sum_x / n_samples
    y_mean = sum_y / n_samples

    centered_xx = (
        sum_xx
        - n_samples * torch.outer(x_mean, x_mean)
    )

    centered_xy = (
        sum_xy
        - n_samples * torch.outer(x_mean, y_mean)
    )

    if isinstance(model.model, RidgeRegressionMapping):
        gamma = float(model.model.gamma)
        ...
        W = torch.linalg.solve(
            centered_xx + gamma * eye,
            centered_xy,
        )

        if not hasattr(model.model, "x_mean"):
            raise RuntimeError(
                "Ridge model needs x_mean for correct inference."
            )

        if not hasattr(model.model, "y_mean"):
            raise RuntimeError(
                "Ridge model needs y_mean for correct inference."
            )

        model.model.x_mean.copy_(
            x_mean.to(model.model.x_mean.dtype)
        )
        model.model.y_mean.copy_(
            y_mean.to(model.model.y_mean.dtype)
        )
        model.model.W.copy_(
            W.to(model.model.W.dtype)
        )

        if hasattr(model.model, "fitted"):
            model.model.fitted.fill_(True)
        else:
            model.model._fitted = True

    elif isinstance(model.model, OrthogonalProcrustesMapping):
...
def train_model(
    ...
    # --- Handle closed-form models (Orthogonal Procrustes, Ridge) ---
    if fit_closed_form_model_if_needed(model, data_module, device):
        # Closed-form model: no SGD training, just save a checkpoint.
        checkpoint_name = (
            f"{extractor_name}_"
            f"{model_type}_"
            f"{feature_key}_"
        ...
        print(
            f"Closed-form model ({model_type}) fitted and saved to {checkpoint_path}",
            flush=True,
        )

        # Optionally log to TensorBoard (single val_loss entry computed on val set)
        logger_name = (
            f"{model_type}_"
            f"{feature_key}_"
            f"{make_safe_name(manipulation)}_"
        ...
        # Compute a single validation loss for logging
        model.eval()
        val_loader = data_module.val_dataloader()
        val_losses = []

        with torch.no_grad():
            for orig_features, target_features, _ in val_loader:
                orig_features = orig_features.to(device)
                target_features = target_features.to(device)

                outputs = model(orig_features)

                val_loss = model.criterion(
                    outputs,
                    target_features,
                )
        ...
        logger.log_metrics({"val_loss": avg_val_loss}, step=0)
