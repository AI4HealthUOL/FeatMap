...
CONFIG_PATH = PROJECT_ROOT / "config" / "train_mapping_cars_baseline.yaml"
...
def fit_closed_form_model_if_needed(model, data_module, device):
    """
    If model.model is a closed-form mapping (OrthogonalProcrustesMapping or RidgeRegressionMapping),
    fit it on the full training set and return True. Otherwise return False.
    """
    from model_implementations import OrthogonalProcrustesMapping, RidgeRegressionMapping

    if not isinstance(model.model, (OrthogonalProcrustesMapping, RidgeRegressionMapping)):
        return False

    # Ensure the datamodule is set up (train/val splits created)
    if not hasattr(data_module, "paired_train_dataset"):
        data_module.setup(stage="fit")

    # Gather all training pairs
    ...
    with torch.no_grad():
        for batch in train_loader:
            ...
            # orig_features, target_features: [B, C, H, W]
            B, C, H, W = orig_features.shape
            X = orig_features.permute(0, 2, 3, 1).reshape(-1, C).to(device)
            Y = target_features.permute(0, 2, 3, 1).reshape(-1, C).to(device)
            X_list.append(X)
            Y_list.append(Y)
...
def train_model(
    ...
    if fit_closed_form_model_if_needed(model, data_module, device):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_loss = model.criterion(outputs, target_features)
    ...
    # --- Standard SGD training for all other models ---
    logger_name = (
        f"{model_type}_"
        f"{feature_key}_"
        f"{make_safe_name(manipulation)}_"
